...
func GetClusterMemberSchemaVersionsAndAPIExtensions(ctx context.Context, tx *sql.Tx) ([]InternalClusterMemberVersioningInfo, error) {
	sql := "SELECT schema, internal_api_extensions, external_api_extensions FROM internal_cluster_members WHERE NOT role='pending'"
	...
	for rows.Next() {
		...
		var InternalAPIExtensions *string
		var ExternalAPIExtensions *string
		...
		if InternalAPIExtensions == nil || ExternalAPIExtensions == nil {
			ext = nil
		} else {
			ext, err = extensions.NewExtensionRegistryFromList(strings.Split(*InternalAPIExtensions+","+*ExternalAPIExtensions, ","))
			if err != nil {
				return nil, err
			}
		}

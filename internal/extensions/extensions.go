...
func (e *Extensions) Register(newExtensions []string) error {
	...
	for _, extension := range newExtensions {
		if shared.ValueInSlice[string](extension, []string(*e)) {
...
// MarshalJSON implement the json.Marshaler interface to serialize the Extensions struct
// for API responses.
func (e *Extensions) MarshalJSON() ([]byte, error) {
	return json.Marshal([]string(*e))
}

// UnmarshalJSON implement the json.Unmarshaler interface to deserialize the Extensions struct
// from API requests.
...
func (e *Extensions) Scan(value interface{}) error {
	...
	bytes, ok := value.([]byte)
	if !ok {
		return errors.New("type assertion to []byte failed, incompatible type")
	}

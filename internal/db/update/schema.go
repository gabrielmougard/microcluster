...
// It always applies all the overrides in the order they were added and do not
// trust the current database schema version as we don't know which version correspond to an internal
// update (related to MicroCluster) and which one correspond to an external update.
// It's the responsibility of the caller to ensure that the overrides are correct, whatever the state
// of the database.

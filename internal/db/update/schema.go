...
func (s *SchemaUpdate) ExecuteUpdatesOverride(ctx context.Context, current int, tx *sql.Tx) (int, error) {
	for _, updateOverride := range s.updatesOverride {
		if current >= updateOverride.migrationIdx {
			logger.Errorf("GABRIEL - on est la 1: current - %d , updateOverride.migrationIdx - %d", current, updateOverride.migrationIdx)
			//continue
		}

		err := updateOverride.update(ctx, tx)
		if err != nil {
			logger.Errorf("GABRIEL - Failed to apply update override: %v", err)
			//return fmt.Errorf("failed to apply update override: %w", err)
		}

		current = updateOverride.migrationIdx

		statement := `INSERT INTO schemas (version, updated_at) VALUES (?, strftime("%s"))`
		_, err = tx.ExecContext(ctx, statement, current)
		if err != nil {
			logger.Errorf("GABRIEL - on est la 2")
			return -1, fmt.Errorf("failed to insert version %d: %w", current, err)
		}
	...
	logger.Errorf("GABRIEL - on est la 3 with current - %d", current)
